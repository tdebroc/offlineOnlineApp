...
public class GenerateRandomEntity extends HttpServlet {
	...
	/**
	 * function to generate a new Entity based on its name (assuming it exists
	 * already in EntityProperty)
	 * @param entityName : name of the Entity to generate
	 */
	public void retrievePropertiesFromEntityKind(String entityName) {
		Query q = new Query("EntityProperty").addFilter("EntityName",
				Query.FilterOperator.EQUAL, entityName);
		...
		for (Entity res : pq.asIterable()) {
			...
			if (type.equals("Number")) {
				...
				if (minValue != null && maxValue != null) {
					value = String.valueOf(Math.round(Math.random()
