define([ "jquery", "Table" ], function($, Table) {
	...
	function DataBaseManager(modelName) {
		...
		this.checkSynchronization = function() {
			$.ajax(
					{
						'url' : "checkSynchronization?timestamp="
								+ this.getTimeStampDBVersion(MODEL_NAME) + "&"
								+ "entityKind=" + MODEL_NAME
					}).done(this.checkSynchronizationSuccess.bind(this)).fail(
					this.checkSynchronizationFailure.bind(this));
		...
		this.synchronizeLocalDatabase = function(changesDatas) {
			...
			for (var i = 0; i < changesDatas.length; i++) {
				...
				if (change['changeType'] == "update") {
					allUpdateChanges.push(JSON.parse(change["entityJson"]));
				} else if (change['changeType'] == "delete") {
					allRemoveChanges.push(JSON.parse(change["entityJson"]));
				} else if (change['changeType'] == "insert") {
					allInsertChanges.push(JSON.parse(change["entityJson"]));
				}
		...
		this.addUpdateToOfflineWaitingList = function(jsonEntity) {
			var isAnInsertedEntity = this.verifyIfInsertedEntity(jsonEntity);
		...
		this.verifyIfInsertedEntity = function(jsonEntity) {
			...
			for (var i = 0; i < offlineWaitingList.length; i++) {
				if (offlineWaitingList[i]['entityJson'][ENTITY_KEY_PROPERTY_NAME] == jsonEntity[ENTITY_KEY_PROPERTY_NAME]) {
		...
		this.getOfflineWaitingList = function(modelName) {
			var offlineWaitingList = localStorage["offlineWaitingList"
					+ modelName];
		...
		this.setOfflineWaitingList = function(modelName, jsonChanges) {
			localStorage["offlineWaitingList" + modelName] = JSON
					.stringify(jsonChanges);
		}
		...
		this.launchOfflineWaitingListSynchronization = function() {
			// TODO() : parses all local Storage key to look all
			// offlineWainting
			// list.
			var offlineWaitingList = this.getOfflineWaitingList(MODEL_NAME);
			...
			$.get("pushChanges?changes=" + JSON.stringify(offlineWaitingList))
					.done(this.callbackWaitingListSynchronization.bind(this)).fail(
							function(data) {
								console.log(data)
							});
		}
		 ...
		 * 
		 * @param {JSON}
		 *          data Answer from update Entities callback.
		 */
		...
		/**
		 * @param {String}
		 *          entityKind The entity kind from which we want the db
		 *          version.
		 * @return {Number} the timestamp designing the DB Version.
		 */
		this.getTimeStampDBVersion = function(entityKind) {
			var timestamp = localStorage.getItem("TimestampDBVersion-"
					+ entityKind)
		...
		/**
		 * @param {String}
		 *          entityKind The entity kind from which we want to set the db
		 *          version.
		 * @param {Numbre}
		 *          the timestamp designing the DB Version
		 */
		this.setTimeStampDBVersion = function(entityKind, timestampValue) {
			localStorage.setItem("TimestampDBVersion-" + entityKind,
					timestampValue)
		}
		...
		this.updateEntity = function(jsonEntity) {
			...
			}).done(this.successUpdateEntity.bind(this)).fail(
					this.failUpdateEntity.bindWithParams(this, [ jsonEntity ]));
		 ...
		 * 
		 * @param {JSON}
		 *          data Answer from the updateEntity service.
		 */
		...
		/**
		 * @param {JSON}
		 *          jsonEntity Answer from update Entities callback.
		 */
		...
		this.updateLocalDBWithArray = function(jsonEntities) {
			...
			// TODO: Improves perfs here by an hashmap containing lines
			// to udpate.
		...
		this.generateRandomEntity = function(modelName) {
			...
			$.get('/generateRandomEntity?entityName=' + modelName).done(
					this.successGenerateRandomEntity.bind(this)).fail(
					this.failGenerateRandomEntity.bind(this));
		}
		...
		this.getUniqueTemporaryKeyIndex = function(modelName) {
			...
			localStorage["temporaryKeyIndex-" + modelName] = parseInt(temporaryKeyIndex) + 1;
		 ...
		 *          {String}
		 * @param modelName
		 *          {Number} The new Value of temporary Key;
		 */
		...
		this.insertEntity = function(jsonEntity) {
			...
			$.get("/insertEntity?jsonEntity=" + JSON.stringify(jsonEntity)).done(
					this.successInsertEntity.bind(this)).fail(
					this.failInsertEntity.bindWithParams(this, [ jsonEntity ]));
		}
		...
		this.insertEntityArrayLocalDB = function(jsonEntitiesArray) {
			...
			var entities = this.getJSONEntitiesFromLocalStorage(modelName);
			jsonEntitiesArray = this.removeExistingEntities(entities,
					jsonEntitiesArray);
		...
		/**
		 * Updates temporary keys with real keys after inserting new Entity in
		 * the datastore.
		 */
		...
		this.removeEntity = function(key) {
			$.ajax(
					{
						'url' : "removeEntity?entityKey=" + key + "&entityKind="
								+ MODEL_NAME
					}).done(this.removeEntitySuccess.bind(this)).fail(
					this.removeEntityFailure
							.bindWithParams(this, [ key, MODEL_NAME ]));
		 ...
		 * 
		 * @param {String}
		 *          key The key of the entity we want to remove.
		 * @param {String}
		 *          modelName Name of the model from which we want to remove the
		 *          entity.
		 */
		...
		this.getJSONEntitiesFromLocalStorage = function(modelName) {
			return $
					.parseJSON(this.getStringEntititesFromLocalStorage(modelName));
		}
		...
		/**
		 * @param {JSON}
		 *          result from the service to get all Datas.
		 */
