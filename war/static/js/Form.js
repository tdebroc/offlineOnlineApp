define([ 'jquery', 'DBManager' ],function($, dbManager) {
	...
	function Form(dbManager) {
		 ...
		 * 
		 * @param {Array[String]}
		 *          fields List of fields displayed.
		 * @param {String}
		 *          entityKind Kind of the entity.
		 * @param {String}
		 *          key Key of the entity.
		 * @param {Array[String]}
		 *          opt_dataLine Value of the field.
		 */
		this.buildForm = function(fields, entityKind, key, opt_dataLine) {
			...
			for (var i = 0; i < fields.length; i++) {
				var input = this.buildInput('text', fields[i],
						opt_dataLine ? opt_dataLine[i] : "", fields[i]);
				table.append(input);
			}
		...
		this.launchesUpdate = function() {
			console.log("launches Updates : "
					+ JSON.stringify(this.serializeForm()));
